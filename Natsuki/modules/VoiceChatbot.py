...
async def Lycia(_, message):
    if len(message.command) < 2:
        await message.reply_text("Dragon Warrior AI Voice Chatbot")
        return
    text = message.text.split(None, 1)[1]
    lycia = text.replace(" ", "%20")
    m = await message.reply_text("Dragon Warrior Is Best...")
    try:
        L = await fetch(
            f"https://api.affiliateplus.xyz/api/chatbot?message={lycia}&botname=@TheDragonWarriorBot&ownername=RGamer_s&user=1"
        )
        chatbot = L["message"]
